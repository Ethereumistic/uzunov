...
export default function Footer() {
    ...
    const services = [
        { label: "Архитектура", href: "/#services-arch", icon: Building2 },
        ...
        { label: "Консултиране", href: "/#services-consultant", icon: Handshake },
        { label: "3D Анимация и VR", href: "/#services-3d-vr", icon: Box },
        { label: "Управление на проекти", href: "/#services-pm", icon: ClipboardCheck },
    ]
    ...
    return (
        <footer className="relative mt-32 border-t border-black/3 bg-linear-to-b from-transparent to-white/40 py-8 overflow-hidden">
            {/* Background Accent */}
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[1200px] h-[400px] bg-linear-to-t from-black/2 to-transparent rounded-full blur-3xl -z-10 pointer-events-none" />
            ...
            <div className="mx-auto max-w-7xl px-6">
                <div className="grid grid-cols-2 gap-12 lg:grid-cols-12 lg:gap-8">
                    ...
                    <div className="col-span-2 lg:col-span-4 flex flex-col items-start gap-8 order-1">
                        ...
                        <p className="max-w-sm text-base leading-relaxed text-muted-foreground/80 font-medium italic">
                    ...
                    <div className="col-span-1 lg:col-span-2 flex flex-col gap-8 order-2">
                        <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 select-none font-display">
                            Студио
                        </h3>
                        <nav className="flex flex-col gap-4">
                            {quickLinks.map((link) => (
                                <Link
                                    key={link.label}
                                    to={link.href as any}
                                    className="group text-[12px] font-semibold text-muted-foreground hover:text-foreground transition-all flex items-center gap-1 w-fit"
                                >
                    ...
                    <div className="col-span-1 lg:col-span-2 flex flex-col gap-8 lg:pl-4 order-3 lg:order-4">
                        ...
                        <div className="flex flex-col gap-4">
                            <a href="mailto:office@uzunov.bg" className="flex items-center gap-3 text-[12px] font-semibold text-muted-foreground hover:text-foreground transition-colors group w-fit whitespace-nowrap">
                                ...
                                <span>office@uzunov.bg</span>
                            </a>
                            <a href="tel:+359888000000" className="flex items-center gap-3 text-[12px] font-semibold text-muted-foreground hover:text-foreground transition-colors group w-fit whitespace-nowrap">
                                <div className="flex items-center justify-center size-7 rounded-lg bg-black/5 group-hover:bg-primary/10 group-hover:text-primary transition-colors shrink-0">
                                    <Phone className="size-3.5" />
                                </div>
                                +359 888 722 730
                            </a>
                            <div className="flex items-center gap-3 text-[12px] font-semibold text-muted-foreground group">
                        ...
                        <nav className="grid grid-cols-2 gap-x-6 gap-y-4">
                            {services.map((service) => (
                                <Link
                                    ...
                                    className="group text-[12px] font-semibold text-muted-foreground hover:text-foreground transition-all flex items-center gap-2.5 w-fit"
                                >
                                    <div className="flex items-center justify-center size-7 rounded-lg bg-black/5 group-hover:bg-primary/10 group-hover:text-primary transition-colors shrink-0">
                ...
                {/* Bottom Bar */}
                <div className="mt-24 pt-10 border-t border-border flex flex-col md:flex-row items-center justify-between gap-8">
